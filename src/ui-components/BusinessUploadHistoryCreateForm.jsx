...
import { createBusinessUploadHistory } from "../graphql/mutations";
export default function BusinessUploadHistoryCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setValidCount(initialValues.validCount);
    setTotal(initialValues.total);
    setInvalidCount(initialValues.invalidCount);
    setOriginalPath(initialValues.originalPath);
    setInvalidPath(initialValues.invalidPath);
    setErrors({});
  };
  const validations = {
    uploadDate: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: createBusinessUploadHistory.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "BusinessUploadHistoryCreateForm")}
      {...rest}
    >
      <TextField
        label="Upload date"
